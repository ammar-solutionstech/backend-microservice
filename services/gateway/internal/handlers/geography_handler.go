...
func NewGeographyHandler(cfg *config.Config) *GeographyHandler {
	serviceURL := cfg.GeographyServiceHTTP
	if serviceURL == "" {
		if strings.Contains(cfg.GeographyServiceGRPC, "geography-service") {
			serviceURL = "http://geography-service:8008"
		} else {
...
func (h *GeographyHandler) Proxy(w http.ResponseWriter, r *http.Request) {
